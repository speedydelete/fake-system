...
export abstract class RegularFile extends FileObject {

    abstract data: Uint8Array;

    abstract cp(): RegularFile;

    abstract read(encoding?: BufferEncoding, start?: number, length?: number): string;
    abstract read(encoding: 'uint8array', start?: number, length?: number): Uint8Array;
    abstract read(encoding: 'buffer', start?: number, length?: number): Buffer;

    abstract write(data: string, position?: number, encoding?: BufferEncoding): void;
    abstract write(data: TypedArray | DataView | Iterable<any>, offset?: number, length?: number): void;
    
    abstract append(data: DataArg, encoding?: BufferEncoding): void;
...
export class RealRegularFile extends RegularFile {
    ...
    cp(): RealRegularFile {
        return new RealRegularFile(new Uint8Array(this.data), {mode: this.mode, uid: this.uid, gid: this.gid});
    }
    ...
    static import(data: Uint8Array, version: ExportFormatVersion = CURRENT_EXPORT_FORMAT_VERSION): RegularFile {
        ...
        let out = new RealRegularFile(data.slice(10, 10 + info.size), info);
...
export class FunctionBinary extends RegularFile {

    executor: DeviceExecutor;

    constructor(executor: DeviceExecutor, {mode = 0o7440, ...params}: FileParams & {encoding?: BufferEncoding}) {
        super({mode: mode | S_IFREG, ...params});
        this.executor = executor;
    }

    get data(): Uint8Array {
        return encode(`#!/bin/js\n${this.executor}\n;${this.executor.name}(process)`);
    }

    cp(): FunctionBinary {
        return new FunctionBinary(this.executor, {mode: this.mode, uid: this.uid, gid: this.gid});
    }
    ...
    write(data: TypedArray | DataView | Iterable<any>, offset?: number, length?: number): void;
    write(data: DataArg, position?: number, encoding_or_length?: number | BufferEncoding): void {
        throw new TypeError('cannot write to FunctionBinary');
    }
...
export class Directory extends FileObject {
    ...
    write(path: PathArg, data: DataArg, position?: number, encoding_or_length?: number | BufferEncoding): void {
        if (this.exists(path)) {
            ...
            this.link(path, new RealRegularFile(data, {uid: this.uid, gid: this.gid}));
        }
    }
    ...
    addFunctionBinary(path: PathArg, func: DeviceExecutor): void {
        this.link(path, new FunctionBinary(func, {uid: this.uid, gid: this.gid}));
    }
    ...
    static import(data: Uint8Array, version: ExportFormatVersion = CURRENT_EXPORT_FORMAT_VERSION): Directory {
        ...
        for (let i = 0; i < info.size; i++) {
            ...
            if ((meta.mode & S_IFREG) === S_IFREG) {
                file = RealRegularFile.import(fileData);
