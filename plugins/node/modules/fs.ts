...
export function linkSync(existingPath: PathArg, newPath: PathArg): void {
    __fakeNode__.fs.getDir(dirname(parsePathArg(existingPath, __fakeNode_process__.cwd))).link(basename(parsePathArg(newPath, __fakeNode_process__.cwd)), __fakeNode__.fs.get(existingPath));
}
